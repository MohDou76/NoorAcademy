...
function Home({ t, lang, setScreen, setActiveSub, plan, onLocked }) {
  ...
  return (
    <div style={{padding:"24px 20px 100px",direction:t.dir}}>
      ...
      {/* Du'a card */}
      <div onClick={()=>setRevealedDua(true)} style={{background:`linear-gradient(135deg,${D.navy},${D.navyCard})`,border:`1px solid ${D.gold}44`,borderRadius:18,padding:"22px 22px",marginBottom:22,cursor:revealedDua?"default":"pointer",position:"relative",overflow:"hidden",boxShadow:`0 4px 32px ${D.gold}18`}}>
        <GeomPattern opacity={0.06}/>
        <div style={{position:"relative",zIndex:1}}>
          <p style={{fontSize:10,color:D.gold,letterSpacing:2,textTransform:"uppercase",marginBottom:8,fontWeight:700}}>{t.dua_day}</p>
...
function QuizScreen({ t, lang, onBack }) {
  ...
  return (
    <div style={{padding:"24px 20px 100px",direction:t.dir,animation:"fadeUp 0.3s ease"}}>
      ...
      <div style={{background:D.navyCard,border:`1.5px solid ${D.border}`,borderRadius:18,padding:24,marginBottom:18,animation:shake?"shake 0.5s":undefined,boxShadow:`0 4px 24px rgba(0,0,0,0.2)`}}>
        <p style={{fontFamily:"'Amiri',serif",fontSize:20,color:D.cream,margin:0,lineHeight:1.6}}>{q.q}</p>
      </div>
      <div style={{display:"grid",gap:10}}>
        {q.opts.map((opt,i)=>{
    ...
    <div style={{padding:"24px 20px 100px",direction:t.dir,animation:"fadeUp 0.3s ease"}}>
      ...
      <div style={{display:"grid",gap:14}}>
        {DUAS.map((d,i)=>{
          ...
          const c = colors[i%4];
          return (
            <div key={i} onClick={()=>setOpen(isOpen?null:i)} className="card-hover"
              ...
              <div style={{position:"relative",zIndex:1}}>
                <p style={{fontFamily:"'Amiri',serif",fontSize:34,direction:"rtl",color:D.cream,lineHeight:1.5,marginBottom:8}}>{d.arabic}</p>
            ...
            return (
              <div key={lesson.id} onClick={isLocked?onLocked:undefined}
                ...
                <div style={{flex:1}}>
                  ...
                  {/* Arabic preview */}
                  <p style={{margin:"6px 0 0",fontFamily:"'Amiri',serif",fontSize:18,direction:"rtl",color:level.color,lineHeight:1.4}}>{lesson.arabic}</p>
                  <p style={{margin:"2px 0 0",fontSize:11,color:D.muted,fontStyle:"italic"}}>{lesson.example}</p>
                </div>
                {!isDone && !isLocked && (
                  <button onClick={()=>setCompletedLessons(prev=>({...prev,[`${activeLevel}-${lesson.id}`]:lesson.xp}))} className="btn-hover"
  ...
  if (activeCard !== null) {
    ...
    return (
      <div style={{padding:"24px 20px 100px",animation:"scaleIn 0.35s ease",minHeight:"100vh",background:D.midnight}}>
        ...
        <button onClick={()=>setActiveCard(null)} style={{background:"none",border:"none",color:D.gold,fontSize:16,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginBottom:20}}>{backBtn}</button>
        ...
        <div style={{background:`linear-gradient(135deg,${lesson.color}22,${lesson.color}11)`,border:`2px solid ${lesson.color}66`,borderRadius:24,padding:"30px 24px",textAlign:"center",marginBottom:16,boxShadow:`0 8px 40px ${lesson.color}22`}}>
          ...
          <p style={{fontSize:16,color:D.parchment}}>{meaning}</p>
        </div>
        ...
        {/* Big star button */}
        {!earned ? (
          <button onClick={()=>celebrate(lesson.id)} className="btn-hover"
    ...
    <div style={{padding:"24px 20px 100px",animation:"fadeUp 0.3s ease"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        {KIDS_LESSONS.map((lesson,i)=>{
          ...
          const isLocked = plan==="free" && i>=2;
          return (
            <div key={lesson.id} onClick={isLocked?onLocked:()=>setActiveCard(i)} className="card-hover"
        ...
        <div style={{background:D.deep,borderBottom:`1px solid ${D.border}`,padding:"14px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:50}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            ...
            <div style={{display:"flex",gap:3,background:D.navyCard,borderRadius:20,padding:3,border:`1px solid ${D.border}`}}>
              {Object.entries(LANGS).map(([code,v])=>(
                <button key={code} onClick={()=>setLang(code)} style={{background:lang===code?D.gold:"transparent",border:"none",borderRadius:16,padding:"4px 8px",cursor:"pointer",color:lang===code?D.midnight:D.muted,fontSize:10,fontWeight:lang===code?800:400,transition:"all 0.2s",fontFamily:"inherit"}}>
                  {v.flag}
                </button>
              ))}
            </div>
